...
class BlockCrackView(context: Context) : SurfaceView(context), SurfaceHolder.Callback {
    ...
    @Volatile
    private var simulationPaused: Boolean = true
    ...
    private val scorePanelPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
    ...
    @Volatile
    private var crackedCellsTotal: Int = 0
    ...
    fun captureState(): SavedState? {
        val dims = World.Dimensions()
        world.getDimensions(dims)
        val width = dims.width
        val height = dims.height
        if (width <= 0 || height <= 0) return null
        val cells = ByteArray(width * height)
        world.snapshot(cells, dims)
        return SavedState(
            cells = cells,
            width = dims.width,
            height = dims.height,
            config = ConfigSnapshot(
                gridW = WorldConfig.gridW,
                gridH = WorldConfig.gridH,
                noiseStrength = WorldConfig.noiseStrength,
                crackRadiusCells = WorldConfig.crackRadiusCells,
                gravityStepsPerFrame = WorldConfig.gravityStepsPerFrame
            ),
            score = crackedCellsTotal
        )
    }

    fun restoreState(state: SavedState) {
        WorldConfig.gridW = state.config.gridW
        WorldConfig.gridH = state.config.gridH
        WorldConfig.noiseStrength = state.config.noiseStrength
        WorldConfig.crackRadiusCells = state.config.crackRadiusCells
        WorldConfig.gravityStepsPerFrame = state.config.gravityStepsPerFrame
        world.restore(state.cells, state.width, state.height)
        gridWidth = state.width
        gridHeight = state.height
        val needed = state.width * state.height
        ...
        dustParticles.clear()
        jitterFrame = 0
        crackedCellsTotal = state.score
    }

    fun resetWorld() {
        world.ensureSize(WorldConfig.gridW, WorldConfig.gridH)
        world.fillRainbow()
        pathPoints.clear()
        dustParticles.clear()
        jitterFrame = 0
    ...
    private inner class RenderThread : Thread("BlockCrackThread") {
        ...
        override fun run() {
            while (isRunning) {
                ...
                if (canvas != null) {
                    try {
                        val dtForDraw = if (simulationPaused) 0f else delta
                        drawFrame(canvas, movedThisFrame, dtForDraw, stepsTaken)
                    } finally {
    ...
    private fun drawScore(canvas: Canvas, offsetX: Float, offsetY: Float) {
        val score = crackedCellsTotal
        val text = "Blocks removed: $score"
        val margin = 12f * density
        val panelLeft = max(margin, offsetX + margin)
        val panelTop = max(margin, offsetY + margin)
        val metrics = scoreTextPaint.fontMetrics
        val textWidth = scoreTextPaint.measureText(text)
        val textHeight = metrics.bottom - metrics.top
        val rect = RectF(
            panelLeft,
            panelTop,
            panelLeft + textWidth + margin * 2f,
            panelTop + textHeight + margin * 2f
        )
        canvas.drawRoundRect(rect, 12f * density, 12f * density, scorePanelPaint)
        val baseline = rect.top + margin - metrics.top
        canvas.drawText(text, rect.left + margin, baseline, scoreTextPaint)
    }
    ...
    private fun drawStats(canvas: Canvas) {
        ...
        val lines = listOf(
            ...
            "Blocks removed: $crackedCellsTotal",
    ...
    private fun finishCrack() {
        ...
        if (samples.isNotEmpty()) {
            val removed = world.carve(samples)
            if (removed > 0) {
                crackedCellsTotal += removed
            }
        }
        pathPoints.clear()
    }
    ...
    data class ConfigSnapshot(
        ...
        val crackRadiusCells: Int,
        val gravityStepsPerFrame: Int
    )
