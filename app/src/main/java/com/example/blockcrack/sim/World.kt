...
class World(initialW: Int = WorldConfig.gridW, initialH: Int = WorldConfig.gridH) {
    ...
    @Synchronized
    fun restore(from: ByteArray, newWidth: Int, newHeight: Int) {
        require(newWidth > 0 && newHeight > 0) { "Invalid world size" }
        val size = newWidth * newHeight
        require(from.size >= size) { "Snapshot buffer too small" }
        width = newWidth
        height = newHeight
        if (cur.size != size) {
            cur = ByteArray(size)
